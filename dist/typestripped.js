...
const 
// Atoms
WHITESPACE = descr(/(\s|\/\/.*|\/\*[\s\S]*?\*\/)+/y, "whitespace"), IDENTIFIER = descr(/[a-zA-Z_$][0-9a-zA-Z_$]*/y, "identifier"), STRING = descr(/(['"])(?:(?=(\\?))\2.)*?\1/y, "string"), NUMBER = descr(/[+-]?\d+(\.\d+)?([eE][+-]?\d+)?/y, "number"), OPERATOR = descr(/instanceof\b|in\b|[!=]==|\|\|=?|&&=?|[+\-*\/%&|^!=<>]=|[+\-*\/%&|^=<>]/y, "bin-op"), BACKTICK_STRING = descr(/[\s\S]*?(\${|`)/y, "`string`"), EXPRESSION_PREFIX = descr(/\+\+|--|!|~|\+|-|typeof\b|delete\b|await\b/y, "pre-op"), REGEXP = descr(/\/(\\.|[^\/])+\/[gimsuyd]*/y, "regexp"), 
// Other regexes
 ...
 */
export function typestripped(code, debug = false, recover = false) {
