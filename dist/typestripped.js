...
function descr(regexp, string) {
    regexp.toString = () => '<' + string + '>';
    return regexp;
}
const 
// Atoms
WHITESPACE = descr(/(\s|\/\/.*|\/\*[\s\S]*?\*\/)+/y, "whitespace"), IDENTIFIER = descr(/[a-zA-Z_$][0-9a-zA-Z_$]*/y, "identifier"), STRING = descr(/(['"])(?:(?=(\\?))\2.)*?\1/y, "string"), NUMBER = descr(/[+-]?(?:0[xX][0-9a-fA-F]+|0[oO][0-7]+|0[bB][01]+|\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)/y, "number"), OPERATOR = descr(/instanceof\b|in\b|[!=]==|\|\|=?|>>=?|<<=?|&&=?|[+\-*\/%&|^!=<>]=|[+\-*\/%&|^=<>]/y, "bin-op"), BACKTICK_STRING = descr(/[\s\S]*?(\${|`)/y, "`string`"), EXPRESSION_PREFIX = descr(/\+\+|--|!|~|\+|-|typeof\b|delete\b|await\b/y, "unary-op"), REGEXP = descr(/\/(\\.|[^\/])+\/[gimsuyd]*/y, "regexp"), 
// Other regexes
ANYTHING = /[\s\S]/y, ALL_NOT_WHITESPACE = /\S/g, ALPHA_NUM = /^[a-zA-Z0-9]+$/;
/**
 ...
 */
export function typestripped(code, { debug, recover, transformImport } = {}) {
    ...
    let matchOptions = new Set(); // The set of tokens we've tried and failed to match at this `pos`
    ...
    function parseStatement() {
        ...
        if (parseVarDecl() || parseTypeDecl() || parseBlock() || parseExport() || parseEnum() || parseClass() ||
            ...
            parseTry() || parseDeclare() || parseSwitch() || parseExpressionStatement() || eat(';'))
            return true;
        return false;
    }
    function parseExpressionStatement() {
        if (!parseExpressionSeq())
            return false;
        mustEos();
        return true;
    }
    ...
    function parseFunction() {
        ...
        if (!eat('async', 'function') && !eat('function'))
            return false;
        eat(IDENTIFIER);
        parseTemplateDef();
    ...
    function parseArrowFunction() {
        ...
        if (!attempt(() => {
            eat('async');
            parseTemplateDef();
            if (!parseFuncParams() && !eat(IDENTIFIER))
                return false;
            if (skip(':'))
                must(skip(parseType));
            if (!eat('=>'))
    ...
    function parseExpression() {
        ...
        while (true) {
            if (peek('('))
                ...
                must(eat(IDENTIFIER) || parseIndex());
            else if (eat('.'))
                must(eat(IDENTIFIER));
    ...
    function parseLiteralArray() {
        ...
        while (true) {
            if (eat('...'))
                ...
                parseExpression(); // Can be empty: [1 ,,, 2] is valid
            if (!eat(','))
                break;
        }
        must(eat(']'));
    ...
    function parseTemplateArg() {
        ...
        return attempt(() => {
            ...
            if (peek('.') || peek('(') || peek('{') || peek(';') || pos >= code.length)
                return true;
    ...
    function parseMethod(isAbstract = false) {
        ...
        // static { log('init'); }
        statementOutStart = out.length;
        isAbstract || (isAbstract = !!skip('abstract'));
    ...
    ///// Helper functions /////
    function replaceOutput(outPos, text) {
        ...
        if (textLineCount > replacedLineCount)
            throw new Error("Invalid replaceOutput");
        ...
        if (lastReplacedLine.trim() === '')
            out += lastReplacedLine;
        if (debug)
            debugLog('replace output', toJson(replacedText), "by", toJson(out.substr(outPos)));
    }
    function must(result) {
        ...
        for (let m of matchOptions) {
            if (m instanceof ParseError)
                attempts.push(m.message);
            else if (m instanceof Array)
                expect.push(joinTokens(m, ' + '));
            else
                expect.push(m);
        }
    ...
    function recoverErrors(func, required = false) {
        if (attempting || !recover)
            return func();
        let startPos = pos;
        try {
            const res = func();
            if (required && startPos === pos)
                must(false);
            return res;
        }
        catch (e) {
    ...
    function attempt(func) {
        ...
        try {
            ...
            if (result)
                return result;
            if (pos === saved.pos && out === saved.out)
                return false; // no need to revert
            must(false);
        }
        catch (e) {
            if (!(e instanceof ParseError))
    ...
    function wipeStatement() {
        ...
        if (orgText !== newText) {
            if (debug)
                debugLog('wipe output', toJson(orgText));
    ...
    function eat(...whats) {
        ...
        for (let what of whats) {
            ...
            if (typeof what === 'string') {
                ...
                if (code.substr(pos, what.length) === what && (!what.slice(-1).match(ALPHA_NUM) ||
        ...
        if (debug)
            debugLog(skipping ? 'skip' : 'eat', toJson(matched), 'as', joinTokens(whats, ' + '));
        ...
        if (skipping)
            out += matched.replace(ALL_NOT_WHITESPACE, ' ');
        else
            out += matched;
        return result;
    }
